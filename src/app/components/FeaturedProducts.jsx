...
//         if (productRes.status === "fulfilled") {
//           const full = productRes.value.data?.product || productRes.value.data;
//           const vid = full?.vendor?._id || full?.vendor || full?.vendorId?._id || full?.vendorId;
//           if (vid) setVendorId(vid);
//         }
//       } catch (err) {
//         console.error(err);
...
//         {/* Hero image - object‑cover looks good in modal */}
//         <div className="relative h-56 sm:h-72 w-full bg-white">
//           <ProductImage
//             src={getImageUrl(product.images?.[0])}
//             alt={product.name}
//             className="object-cover rounded-t-4xl sm:rounded-t-4xl"
...
//                       onClick={() => setSelectedProduct(product)}
//                     >
//                       {/* 1:1 square container, white background, object-contain + padding */}
...
/* ─────────────────────────────────────────────────────────
   ProductImage component (shared between card & modal)
───────────────────────────────────────────────────────── */
function ProductImage({ src, alt, className }) {
  const [imgSrc, setImgSrc] = useState(src);
  useEffect(() => { setImgSrc(src); }, [src]);
...
/* ─────────────────────────────────────────────────────────
   Quick‑view modal – completely fixed layout
───────────────────────────────────────────────────────── */
function ProductModal({ product, cartQty, onClose, onAdd, onIncrease, onDecrease }) {
  const qty = cartQty(product._id);
  const { user } = useAuth();
  ...
  // Load reviews & full product (to get vendorId)
  useEffect(() => {
    if (!product?._id) return;
    const loadData = async () => {
      setReviewsLoading(true);
  ...
  return (
    <div
      ...
      onClick={(e) => e.target === e.currentTarget && onClose()}
    >
      <div className="relative w-full sm:max-w-lg max-h-[95vh] overflow-y-auto rounded-t-4xl sm:rounded-4xl bg-white shadow-2xl">
        ...
        <div className="p-5 sm:p-6 space-y-6 pb-32">
          ...
          {product.stock !== undefined && (
            <p className="text-xs text-gray-400">Stock: {product.stock ?? "Available"}</p>
          )}
        ...
        {/* ── Sticky CTA ── */}
        <div className="sticky bottom-0 left-0 right-0 bg-white border-t border-gray-100 p-4 shadow-md">
          {qty > 0 ? (
            <div className="flex items-center justify-between bg-brand-green-pale rounded-2xl px-5 py-3">
              <button
                onClick={() => { onDecrease(product); onClose(); }}
                ...
                <ScrollReveal key={product._id} animation="scale-up" delay={i * 100}>
                  ...
                  >
                    ...
                    <div
                      ...
                      onClick={() => setSelectedProduct(product)}
                    >
                      {/* 1:1 square with object-contain + padding */}
                      <div className="relative aspect-square w-full bg-white">
                        <ProductImage
                          src={getImageUrl(product.images?.[0])}
                          alt={product.name}
